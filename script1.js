...
imagesItems.map((item, index) => {
    ...
    index % 2 == 0 ? (item.style.left = "25.5%") : (item.style.left = "26%");
...
titles.map((title, index) => {
    index % 2 == 0 ? (title.style.left = "42.3%") : (title.style.left = "45.5%");
    observer.observe(title);
});

ps.map((p, index) => {
    index % 2 == 0 ? (p.style.left = "45%") : (p.style.left = "49%");
    observer.observe(p);
});

p1s.map((p1, index) => {
    index % 2 == 0 ? (p1.style.left = "27%") : (p1.style.left = "39%");
    observer.observe(p1);
});
...
p3s.map((p3, index) => {
    index % 2 == 0 ? (p3.style.left = "43.55%") : (p3.style.left = "70%");
    observer.observe(p3);
});
